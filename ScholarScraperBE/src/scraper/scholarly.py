    ...
    def fill(self):
        ...
        if self.source == "citations":
            ...
            if soup.find("div", class_="gsc_vcd_title_ggi"):
                self.bib["eprint"] = soup.find("div", class_="gsc_vcd_title_ggi").a[
                    "href"
                ]
            self._filled = True
        elif self.source == "scholar":
...
class Author(object):
    ...
    def __init__(self, __data):
        if isinstance(__data, str):
            ...
            self.url_picture = _HOST + "/citations?view_op=medium_photo&user={}".format(
                self.id
            )
            ...
            self.interests = [
                i.text.strip() for i in __data.find_all("a", class_="gs_ai_one_int")
            ]
...
def main() -> None:
    ...
    for i, researcher in enumerate(cs_researchers):
        ...
        # Nothing has changed so just update the date and move on
        if author["citedby"] == old_data[researcher]["citation_count"]:
            new_data[researcher] = old_data[researcher]
            new_data[researcher]["date"] = datetime.now()
            continue

        new_data[researcher]["citation_count"] = author["citedby"]
        new_data[researcher]["name"] = author["name"]
        new_data[researcher]["id"] = author["id"]
        new_data[researcher]["articles"] = {}

        # Fill the author with publication data
        author.fill()

        for pub in author.publications:

            # Nothing has changed so move on
            if pub["citedby"] == old_data[researcher][pub["title"]["citation_count"]]:
                continue

            # Fill publication
            pub.fill()

            # Loop through articles citing the publication
            cites = author.pub.get_citedby()
            for cite in cites:
                new_data[researcher]["articles"][pub["title"]]["citations"][
                    cite["bib"]["title"]
                ] = cite
